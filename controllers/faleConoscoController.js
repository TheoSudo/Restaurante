...
const faleConoscoController = {
  listarMensagens: async (req, res) => {
    try {
      const mensagens = await Mensagem.findAll({ order: [['id', 'ASC']] });
      ...
      res.render('faleConosco', {
        ...
        isUser: req.session.isLoggedIn && !req.session.user?.isAdmin
      });
    } catch (error) {
  ...
  addMensagem: async (req, res) => {
    ...
    try {
      ...
      await Mensagem.create({ nome, email, mensagem, data: dataSP });
  ...
  showEditForm: async (req, res) => {
    try {
      ...
      if (!mensagem) return res.status(404).send('Mensagem não encontrada.');

      res.render('edit_faleConosco', {
  ...
  deleteMensagem: async (req, res) => {
    try {
      const result = await Mensagem.destroy({ where: { id: req.params.id } });
      if (result === 0) return res.status(404).send('Mensagem não encontrada.');
